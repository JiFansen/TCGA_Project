...
ggplot(dataset,aes(x=component1,y=component2,col=cancertype),main="Rsubread_No_Correction_Plot_All_Gene.png")+geom_point()
dev.off()
...
dataset <- data.frame(example_large_scale$ydata,cancertype=as.character(cancertype[,2]))
colnames(dataset) <- c("component1","component2","cancertype")
png(file="Rsubread_No_Correction_Plot_xCell_Gene.png",width=300, height=100, units='mm',res=1500)
ggplot(dataset,aes(x=component1,y=component2,col=cancertype),main="Rsubread_No_Correction_Plot_xCell_Gene.png")+geom_point()
dev.off()
