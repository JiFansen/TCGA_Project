#library(doParallel); registerDoParallel(); getDoParWorkers(); registerDoParallel(cores=3)
...
#duplicate <- read.table(file="duplicate",sep="\t",header=F)
#duplicate <- as.character(as.matrix(duplicate))
